...
void testHashTable(FileSim& fs, HashTableType type) {
    fs.setHashTableType(type);
    fs.resetCollisions();
    for (int i = 0; i < 100; ++i) {
    ...
    std::cout << type << " Collisions: " << fs.getCollisions() << std::endl;
}


TEST_F(test_x, testHashTable) {
    FileSim ls(1000);

    cout << "Testing Linear Probing: " << endl;
    testHashTable(ls, LINEAR_PROBING);

    FileSim qs(1000);

    cout << "Testing Quadratic Probing: " << endl;
    testHashTable(qs, QUADRATIC_PROBING);

    FileSim cs(1000);

    cout << "Testing Chaining: " << endl;
    testHashTable(cs, CHAINING);
}